...
export class Favorites extends PureComponent {
  render({ favorites, location, isMobile, onSelected }) {
    if (favorites.length == 0) {
      return <div id="fly-to" class="size-s">
    ...
    if (isMobile) {
      return <select id="wsp-select-fav" onChange={(e) => handleSelectChanged(e, onSelected)}>
        <option>Pick a favorite</option>
      {favorites.map((f) => {
            return <option value={`${f.lat}#${f.lon}`} selected={utils.latLon2str(f) == location}>{label(f)}</option>;
          })
        }
        </select>;
    }
