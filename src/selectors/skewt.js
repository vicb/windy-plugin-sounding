...
export const tToPx = createSelector(
  width,
  tMax,
  tMin,
  (width, tMax, tMin) => math.scaleLinear([tMin, tMax], [0, width])
);
...
export const pToPx = createSelector(
  height,
  pMax,
  pMin,
  (height, pMax, pMin) => math.scaleLog([pMax, pMin], [height, 0])
);

export const pToGh = createSelector(
  params,
  p => math.scaleLog(p.level, p.gh)
);
...
export const line = createSelector(
  ...
  height,
  pToPx,
  (tToPx, skew, height, pToPx) =>
    math.line(v => tToPx(v[0]) + skew * (height - pToPx(v[1])), v => pToPx(v[1]))
);

export const pSfc = createSelector(
  elevation,
  params,
  pToGh,
...
export const tSfc = createSelector(
  params,
  pSfc,
  (params, sfcPressure) => {
    const levels = params.level;
...
export const dewpointSfc = createSelector(
  params,
  pSfc,
  (params, sfcPressure) => {
    const levels = params.level;
...
export const tAxisStep = createSelector(
  tMetric,
  m => (m === "°C" ? 10 : 20)
);
